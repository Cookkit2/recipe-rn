  ...
  private async getCurrentProgress(requirement: AchievementRequirement): Promise<number> {
    try {
      switch (requirement.metric) {
        ...
        case "spices_tracked":
          // Need to query the stock table filtered by spice type
          const spiceCategory = await this.categoryRepo.findByName("Spices");
          if (!spiceCategory) return 0;
          return (await this.stockRepo.getStockByCategory(spiceCategory.id)).length;
