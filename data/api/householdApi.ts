 ...
 */
export const fetchCurrentHousehold = async (): Promise<Household | null> => {
 ...
 */
export const fetchInviteInfo = async (code: string) => {
 ...
 */
export const joinHousehold = async (inviteCode: string): Promise<void> => {
  const user = useAuthStore.getState().user;
  if (!user) throw new Error("Not authenticated");
 ...
 */
export const syncSharedStock = async (householdSupabaseId: string): Promise<void> => {
  await householdSyncService.syncHousehold(householdSupabaseId);
};

export const removeMember = async (memberUserId: string): Promise<void> => {
  const user = useAuthStore.getState().user;
  if (!user) throw new Error("Not authenticated");
