...
export const householdApiFunctions = {
  ...
  fetchCurrentHousehold: async (): Promise<Household | null> => {
    ...
    const [myMembership] = (await memberCollection
      .query(Q.where("user_id", user.id), Q.take(1))
  ...
  leaveHousehold: async (householdId: string): Promise<void> => {
    ...
    const stockCollection = database.collections.get("stock");
    // ⚡ Bolt Optimization: Use native DB query instead of fetching all records and filtering in JS
    // Impact: Prevents N+1 memory allocations and reduces JS/Native bridge crossing
  ...
  dissolveHousehold: async (householdId: string, householdSupabaseId: string): Promise<void> => {
    ...
    await database.write(async () => {
      ...
      // ⚡ Bolt Optimization: Filter at database level to avoid loading entire collection
      // Impact: Significantly reduces memory usage when user has many stock items
  ...
  removeMember: async (memberUserId: string): Promise<void> => {
    ...
    const memberCollection = database.collections.get("household_member");
    const [targetMember] = await memberCollection
      .query(Q.where("user_id", memberUserId), Q.take(1))
      .fetch();
