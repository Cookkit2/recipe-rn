...
import CookingHistory, {
  type CookingHistoryData,
} from "../models/CookingHistory";
...
export class CookingHistoryRepository extends BaseRepository<CookingHistory> {
  ...
  async getCookingHistory(
    options: CookingHistorySearchOptions = {}
  ): Promise<CookingHistory[]> {
  ...
  > {
    const allHistory = await this.collection
      .query(Q.sortBy("cooked_at", Q.desc))
      .fetch();

    // Group by recipe and get unique recipes
    const recipeMap = new Map<
      string,
      { lastCookedAt: number; cookCount: number }
    >();
  ...
  > {
    ...
    const recipeMap = new Map<
      string,
      { cookCount: number; lastCookedAt: number }
    >();
  ...
  async getCookingStats(): Promise<{
    ...
    const averageRating =
      recordsWithRating.length > 0
        ? recordsWithRating.reduce((acc, r) => acc + (r.rating || 0), 0) /
          recordsWithRating.length
        : null;
