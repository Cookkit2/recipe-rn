...
export class DatabaseFacade {
  ...
  private generatePantryHash(items: { name: string; synonyms: string[] }[]): string {
    ...
    // For small arrays, this is much faster than JSON.stringify or crypto hashing
    let hash = `${items.length}:`;
    // We only hash the first 20 and last 20 items to keep it very fast,
    // plus the total count. This is a heuristic but good enough for caching
    // during sequential getAvailableRecipes calls.
    ...
    if (items.length > 40) {
      for (let i = items.length - 20; i < items.length; i++) {
        const item = items[i];
        if (item) hash += item.name + "|";
      }
    }
