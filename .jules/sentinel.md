...
**Learning:** Supabase / PostgREST translates `.ilike()` directly to SQL `ILIKE` and supports `*` as a wildcard alias for `%`. Even when parameterized, wildcard characters act as wildcards in the search pattern, which can lead to unauthorized pattern matching or slow query DoS.
