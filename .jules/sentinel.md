...
## 2026-09-25 - Fix PostgREST wildcard injection
**Vulnerability:** User input passed directly to .ilike() in Supabase/PostgREST is vulnerable to wildcard injection because PostgREST supports `*` as an alias for `%`.
**Learning:** Unsanitized wildcard searches can cause Denial of Service (DoS) or unexpected data leaks. The `*` character must also be explicitly escaped alongside `%` and `_`.
**Prevention:** Always verify type and escape `%`, `_`, `\`, and `*` using `str.replace(/[%_*\]/g, "\$&")` before passing input to .ilike() or .like().
