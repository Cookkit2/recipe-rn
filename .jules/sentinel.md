...
## 2025-02-28 - Insecure Random Number Generation for Invite Codes
**Vulnerability:** Weak `Math.random()` was used for generating invite codes in `utils/invite-code.ts`.
**Learning:** This makes invite codes predictable, potentially allowing unauthorized users to guess codes and join households.
**Prevention:** Always use a CSPRNG like `expo-crypto`'s `getRandomBytes` for generating security-sensitive tokens, and implement rejection sampling when converting to string formats to prevent modulo bias.
