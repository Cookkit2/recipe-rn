...
## 2024-07-02 - Use secure random generation for ID keys
**Vulnerability:** Found `Math.random()` being used to generate surrogate IDs.
**Learning:** `Math.random()` is not cryptographically secure, which could allow attackers to guess IDs.
**Prevention:** Always use `Crypto.randomUUID()` or `Crypto.getRandomBytes` from `expo-crypto` for secure IDs.
