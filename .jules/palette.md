...
## 2026-09-20 - Pressable accessibilityLabel overriding child elements
**Learning:** Using `accessibilityLabel` on a container element like `Pressable` completely overrides the screen reader's reading of any child text elements. This is an anti-pattern when the container includes text that must be read (like instructions or review text).
**Action:** Change `accessibilityLabel` to `accessibilityHint` on these containers so the inner text is read followed by the hint. Or remove the label completely.
