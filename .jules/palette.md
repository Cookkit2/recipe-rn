...
## 2024-05-21 - Explicit Accessibility States on Interactive Elements\n**Learning:** Found that relying solely on the `disabled` prop for React Native `Pressable` components does not guarantee screen readers will announce the disabled state. Users rely on explicit `accessibilityState` to understand when a button is non-interactive (e.g., during network requests).\n**Action:** Always map functional state props (like `disabled={isPending}`) to their corresponding accessibility equivalents, such as `accessibilityState={{ disabled: isPending }}`, especially for destructive actions like delete or discard.
