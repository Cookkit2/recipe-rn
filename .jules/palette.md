...
## 2024-07-06 - Modal Backdrop Accessibility
**Learning:** Custom backdrop overlay components in React Native (like `BaseModal`) using `Pressable` for dismiss actions require explicit `accessibilityRole="button"`, `accessibilityLabel`, and `accessibilityHint` to communicate their interaction model to screen readers. Conversely, internal structural `Pressable` components used purely to block tap propagation (e.g. `e.stopPropagation()`) must be explicitly hidden from screen readers using `importantForAccessibility="no"` to avoid confusing users with empty, meaningless interactive elements.
**Action:** Always add semantic accessibility attributes to full-screen backdrop buttons and hide functional wrappers when creating custom modals or overlays in React Native.
