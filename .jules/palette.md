...
## 2026-07-21 - Accessible Loading States in React Native
**Learning:** In React Native, simply disabling a button and showing an ActivityIndicator when loading causes screen readers to only announce it as 'disabled', losing the context that an action is processing.
**Action:** To provide accurate feedback for screen readers during async operations, explicitly add `busy: isLoading` to the component's `accessibilityState` object, which maps to `aria-busy`.
