...
## 2024-10-18 - Button Loading State Accessibility
**Learning:** When a button is in a loading state, setting `disabled: true` isn't enough. Screen readers need `busy: true` in `accessibilityState` to properly announce that the component is actively processing (mapping to aria-busy).
**Action:** Always include `busy: isLoading` alongside `disabled` in accessibilityState for interactive elements with loading states.
