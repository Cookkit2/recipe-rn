...
## 2024-06-12 - Accessibility Role and Labels on Complex Touch Targets\n**Learning:** Found several complex interactive cards and list items wrapping `Pressable` components that lacked `accessibilityRole="button"` and descriptive `accessibilityLabel` attributes. Without these, screen readers announce them genericly or focus on individual text elements instead of announcing the entire component's purpose and state.\n**Action:** When creating complex interactive components (like cards or list items) with `Pressable`, always explicitly add `accessibilityRole="button"` and build a comprehensive `accessibilityLabel` that incorporates the dynamic data of the item (e.g., title, state, count).
