...
## 2024-05-24 - Checkbox Accessibility Labels
**Learning:** When using `accessibilityRole="checkbox"`, set the `accessibilityLabel` to describe the item itself (e.g., 'Step 1: Preheat oven') rather than the action ('Mark step complete'). Describe the interaction action in `accessibilityHint` to prevent screen readers from outputting redundant/confusing states (e.g., 'Mark step complete, checked, checkbox').
**Action:** Ensure checkboxes have labels describing the item and hints describing the action.
