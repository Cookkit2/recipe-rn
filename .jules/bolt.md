...
## 2024-07-24 - Replace reduce with for-loop in critical hot paths
**Learning:** `Array.prototype.reduce()` creates a closure for the callback function on every invocation, generating avoidable garbage collection pressure. This is especially problematic in computationally intensive code like sorting algorithms (`O(N*log(N))`), where functions like `CompositeRankingStrategy.score` are called repeatedly for every item comparison.
**Action:** When a method is identified as a hot path (e.g., inside a sort or a heavy loop), manually replace `.reduce()` with a standard `for` loop to avoid closure allocation.
