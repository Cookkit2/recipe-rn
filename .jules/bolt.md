...
## 2025-02-28 - Optimize WatermelonDB Array Aggregations
**Learning:** Fetching all records for a repository using `.findAll()` instantiates WatermelonDB `Model` objects for every record in the table, which is memory-intensive and slow for large datasets, suffering from closure overhead and bridge congestion when calculating bulk statistics.
**Action:** When performing aggregate operations on WatermelonDB collections (e.g., getting statistics over all records), use `.unsafeFetchRaw()` instead of `.fetch()` or `.findAll()` to bypass ORM model instantiation, significantly reducing memory footprint and processing latency. Use raw SQLite snake_case column names to access the data.
