...
## 2023-10-25 - FlatList Optimization
**Learning:** Inline `renderItem` functions and missing virtualization props (`removeClippedSubviews`, `initialNumToRender`, etc.) in React Native `FlatList`s cause unnecessary re-renders of all items when the parent component updates, leading to performance bottlenecks in lists.
**Action:** Always extract `renderItem` using `useCallback` and configure virtualization props for `FlatList` components.
