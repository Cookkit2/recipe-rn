...
## 2026-09-13 - Batch conversion N+1 inside map
**Learning:** Found an anti-pattern in `getAvailableRecipesCore` where a batch database conversion function (`convertDbRecipesToUIRecipesBatch`) was being called inside a loop for single item arrays. This effectively downgrades the batch function to an N+1 query loop, blocking the JS thread and creating massive overhead for rendering `partiallyCanMake` lists.
**Action:** When working with related WatermelonDB models that require UI conversion, always map out the child array, run the batch conversion function once over the entire subset, build an in-memory Map keyed by ID, and then construct the final array.
