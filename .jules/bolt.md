...
## 2025-02-28 - Optimize batch record processing by replacing O(N*M) lookups with O(N+M) Map lookups
**Learning:** Using `.filter()` inside a loop over another array (e.g., finding related records for each item in a batch) creates an O(N*M) time complexity trap. This causes a massive performance penalty when batch sizes grow, allocating countless intermediate arrays and destroying execution time.
**Action:** When batch processing relational data, replace nested `array.filter` searches with a single pass `for` loop that groups the related items into a `Map<ID, Item[]>`. Then, iterate through the parent array using O(1) map lookups, achieving O(N+M) time complexity and eliminating redundant array allocations.
