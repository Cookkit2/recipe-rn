...
## 2026-06-13 - Optimize sequential queries in mapping fetch\n**Learning:** Sequential await queries inside loops effectively bottleneck database throughput, especially mapped iterations fetching multiple relational entities simultaneously (steps, ingredients). Bringing queries outside and using Promise.all dramatically reduces I/O wait times and improves application rendering/fetching logic performance when dealing with high relational complexities.\n**Action:** Parallelize operations early using array grouping and filtering with 'for' loops (over functional iterators where performance is critical), fetching mapped item metadata externally before associating to reduce nested promise calls inside loops.
