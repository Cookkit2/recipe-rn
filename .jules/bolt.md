...
## 2026-09-08 - Optimize hot path filter loops\n**Learning:** In React Native frontend applications, optimize frequently executed hot paths (such as text search filters running on every keystroke) by replacing array methods like `.filter()` and `.map()` with standard `for` loops to eliminate closure allocation overhead and reduce garbage collection pressure.\n**Action:** Proactively convert map/filter chains to for loops in text search utilities
