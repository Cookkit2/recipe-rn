...
## 2026-09-13 - Avoid Reduce Closures for Performance
**Learning:** Repeated calls to `Array.prototype.reduce()` allocate closure functions inside loops and map over arrays dynamically, causing noticeable performance overhead, allocating unnecessary intermediate garbage, and increasing GC cycles.
**Action:** Replace `Array.prototype.reduce()` with standard `for` loops in performance-critical sections or repositories to traverse arrays directly and reduce memory allocations.
