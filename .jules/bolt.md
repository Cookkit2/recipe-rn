...
## 2024-02-12 - Eliminate Nested Array Mapping in Grocery List Hot Path\n**Learning:** In `hooks/queries/useGroceryList.ts` and `hooks/useIngredientMatcher.ts`, loops allocating `pantryItem.synonyms?.map(...)` on every iteration mapping unmatched ingredients against pantry items causes massive array allocation penalties and GC pressure.\n**Action:** Pass the `synonyms` array directly to `isIngredientMatch`, taking care to fallback to `[]` when `undefined` via `|| []`. The function natively handles `(string | { synonym: string })[]`, and avoiding the `.map()` prevents unnecessary array allocations and garbage collection pressure.
