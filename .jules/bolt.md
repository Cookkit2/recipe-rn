...
## 2024-03-02 - SQLite GROUP BY for aggregations over JS Map overhead
**Learning:** Fetching raw SQL records and aggregating metrics like average rating in JavaScript using Maps incurs heavy JS iteration and GC overhead. Leveraging native SQLite `GROUP BY` logic directly drops computation time significantly since only pre-aggregated values are sent back across the bridge.
**Action:** When computing aggregates across a whole table (e.g. sum, count, average), use `Q.unsafeSqlQuery` with `GROUP BY` in WatermelonDB combined with `.unsafeFetchRaw()` to let the database handle the aggregation rather than looping over all records in Javascript.
