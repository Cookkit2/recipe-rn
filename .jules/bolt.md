...
## 2024-03-05 - Avoid FlatList for small static grids
**Learning:** Using `FlatList` with `scrollEnabled={false}` for a small, static grid of known size (e.g., 3 columns of segmented buttons) introduces unnecessary virtualization and lifecycle overhead compared to standard React array mapping, causing slower render times and higher memory allocation on page load.
**Action:** Use a standard `View` wrapper with flexbox classes (`flex-row flex-wrap`) mapping over items for small static lists instead of `FlatList` to optimize render cycle overhead.
