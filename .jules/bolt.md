## 2024-03-10 - O(N^2) Filter Elimination

**Learning:** When generating unique combinations or filtering arrays based on multiple object properties in a loop (like finding unique measurement units across a grocery list), chaining `.filter()` and `.find()` or using `Array.prototype.some` inside a `reduce` function creates O(N^2) complexity.
**Action:** Replace nested loops by accumulating unique strings in a `Set`. Use `Set.has()` for O(1) checks during the loop and return `Array.from(set)` at the end to significantly reduce execution time.

## 2024-04-18 - Avoid array iteration in map

**Learning:** In highly repetitive data transformation functions (like calculating matching ingredients against a recipe), chaining multiple Array methods like `map()`, `filter()`, and `reduce()` creates significant unnecessary overhead by allocating intermediate arrays.
**Action:** Consolidate multiple passes over arrays into a single pass using a standard `for` loop, or maintain variables outside the loop (e.g. `let maxScore = 0`) instead of pushing to arrays. When filtering arrays, use a single `for` loop that conditionally pushes to a final array rather than chaining array methods.
