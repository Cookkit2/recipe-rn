...
## 2025-03-02 - [WatermelonDB N+1 fetch optimization]
**Learning:** Using WatermelonDB `.query().fetch().find()` or `.filter()` loads the entire collection into JS memory and incurs heavy JS/Native bridge crossings, leading to massive memory usage and poor performance when tables grow.
**Action:** Use native DB query clauses like `Q.where` and `Q.take(1)` instead.
