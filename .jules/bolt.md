...
## 2026-06-13 - Optimize Database Query In Loop\n**Learning:** Avoid executing WatermelonDB queries (like `await collection.query().fetch()`) inside loops. It triggers N+1 problems across the JS/native bridge, causing slow I/O and excessive memory allocations/garbage collection for JS objects that are immediately discarded.\n**Action:** Fetch all required records once before the loop and build a `Map` keyed by the matching property (e.g. `supabaseId`) for O(1) lookups during iteration.
