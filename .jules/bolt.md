...
## 2025-01-20 - Replace nested array.filter searches with O(1) hash map lookups
**Learning:** Replaced O(N*M) nested `.filter()` searches over large arrays with pre-computed Map lookups inside `buildIngredientResultMap`.
**Action:** When batch processing relational mapping logic with potentially large arrays, extract the iteration to pre-compute maps (O(N) operations) to reduce overall computational complexity from O(N*M) to O(N+M) and avoid nested loops.
