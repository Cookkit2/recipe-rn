...
**Action:** Extract all elements that require processing into a single array, call the batch method once, and use a `Map` to re-associate the processed UI models with their original metadata (like completion percentages) before returning. Always double-check lambda syntax when using methods like `filter` (i.e. remember the `=>` in `(item) => ...`) to avoid compilation errors.
