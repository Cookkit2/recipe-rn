...
## 2024-07-17 - Avoid FlatList for small static grids
**Learning:** Using `FlatList` with `scrollEnabled={false}` inside a `ScrollView` for small, static grids (like preferences or short ingredient lists) introduces unnecessary virtualization overhead (such as `VirtualizedList` initialization and layout measurement calculations) without any benefits. Mapped `View`s are significantly faster and use less memory in these scenarios.
**Action:** Replace `FlatList` with mapped `View` elements utilizing flexbox for static grid layouts where scrolling is not required.
