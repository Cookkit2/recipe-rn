...
## 2026-09-26 - Memoize Component State Derivation and Prefer For-Loops over Reduce
**Learning:** In React components like `AchievementsScreen`, performing complex array derivations (like grouping achievements by category using `.reduce()`) directly in the render body creates unnecessary closures and intermediate array allocations on every render.
**Action:** Always wrap expensive derived state operations in a `useMemo` hook. Furthermore, in performance-critical paths or large datasets, replace `.reduce()` with standard `for` loops to minimize garbage collection pressure from intermediate object/function allocations.
