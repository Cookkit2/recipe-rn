...
## 2025-06-16 - Prevent N^2 Memory Footprint when Filtering Users in WatermelonDB
**Learning:** Calling `await collection.query().fetch()` to load the entire `household_member` table into the JS thread followed by `array.find(m => m.userId === id)` causes significant memory bloat and creates an O(N^2) bottleneck. WatermelonDB must serialize every single member object across the bridge, only for JS to discard all but one.
**Action:** Always delegate exact-match lookups to SQLite natively using `collection.query(Q.where("user_id", id)).fetch()`. This completely removes the bridging and array allocation overhead by only returning the specific record needed.
