...
## 2024-07-29 - Avoid FlatList for small static grids
**Learning:** Using `FlatList` with `scrollEnabled={false}` for small, static layouts (like appliance grids or ingredient lists) within a React Native `ScrollView` introduces unnecessary layout, batching, and virtualization initialization overhead for no benefit. It can also cause nested scroll view warnings.
**Action:** Always map over arrays directly into a flex container (`<View className="flex-row flex-wrap">`) when lists are statically small or non-scrollable to dramatically reduce mount time and bridge allocations.
