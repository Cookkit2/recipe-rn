...
export const TextLoop = forwardRef<TextLoopRef, TextLoopProps>(function TextLoop(
  {
    children,
    className,
    interval = 2,
  ...
  const updateIndex = useCallback(
    (newIndex?: number) => {
      setCurrentIndex((current) => {
        const next = newIndex !== undefined ? newIndex : (current + 1) % items.length;
        onIndexChange?.(next);
        return next;
      });
    },
  ...
  const animateToNext = useCallback(() => {
    ...
    translateY.value = withTiming(-20, CURVES["expressive.fast.spatial"], (finished) => {
      "worklet";
      if (finished) {
        // Update index on main thread
        scheduleOnRN(updateIndex);
  ...
  const animateToIndex = useCallback(
    (index: number) => {
      ...
      translateY.value = withTiming(-20, CURVES["expressive.default.spatial"], (finished) => {
        "worklet";
        if (finished) {
          // Update index on main thread
          scheduleOnRN(updateIndex, index);

          // Reset position and start enter animation
          translateY.value = 20;
          opacity.value = withTiming(1, CURVES["expressive.default.effects"]);
          translateY.value = withTiming(0, CURVES["expressive.default.spatial"]);
        }
      });
    },
  ...
  return (
    <View {...props} style={style} className={cn("relative overflow-hidden", className)}>
      <Animated.View style={animatedStyle}>{items[currentIndex]}</Animated.View>
    </View>
  );
});
