  ...
  const content = (
    <Animated.View className="flex-column items-start p-3" style={[animatedStyle]}>
      <Pressable
        ...
        accessibilityLabel={`${wasteLog.stock?.name || "Unknown Item"}, ${formatQuantity()} wasted on ${wasteLog.formattedWasteDate}${wasteLog.hasReason ? `, Reason: ${getReasonText()}` : ""}${wasteLog.hasEstimatedCost ? `, Estimated Cost: ${wasteLog.formattedCost}` : ""}`}
      >
