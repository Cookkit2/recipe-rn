...
const Gauntlet: React.FC<InfinityGauntletProps> = ({
  children,
  ...
  async function handleSnap(
    ...
    try {
      if (!imageDataArrayRef.current?.length) {
        ...
        const pixelArr = await FileHelper.base64PNGToByteArray(lImgBase64); //await FileHelper.getCanvasImageData(canvas);
          ...
          ...particleRefs.current.map(async (ref, idx) => {
            if (ref.current) {
              ...
              try {
                ...
                console.error("handleSnap error --,>", e);
              }
            }
