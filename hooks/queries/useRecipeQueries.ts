...
// ⚡ Bolt Optimization: Memoize average ratings calculation
// Impact: Reduces O(N) recalculations on identical cooking history arrays
const ratingsCache = new WeakMap<any[], Map<string, number>>();

export function calculateAverageRatings(cookingHistory: any[]): Map<string, number> {
  if (ratingsCache.has(cookingHistory)) {
    return ratingsCache.get(cookingHistory)!;
  }
  ...
  ratingsCache.set(cookingHistory, ratingsMap);
  return ratingsMap;
}
