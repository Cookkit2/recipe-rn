...
export default function NutritionReportScreen() {
  ...
  const stats = useMemo(() => {
    ...
    // ⚡ Bolt Performance Optimization: Consolidate multiple sequential reduces into a single loop to reduce time complexity from O(k*N) to O(N) and minimize closure allocation overhead.
    let totalCalories = 0;
    let totalProtein = 0;
    let totalCarbs = 0;
    let totalFat = 0;
    ...
    if (previousWeekData && previousWeekData.length > 0) {
      ...
      if (prevDaysWithData.length > 0) {
        // ⚡ Bolt Performance Optimization: Consolidate multiple sequential reduces into a single loop to minimize closure allocation overhead
